...
def get_calib_dataloader(experiment_type: str, info: Dict[str, Any], net: Literal['im2im', 'qutcc', 'im2im_deep'], 
...
def get_test_dataloader(experiment_type: str, info: Dict[str, Any], net: Literal['im2im', 'qutcc', 'im2im_deep'], 
...
def get_checkpoint_info(experiment_type: str, info: Dict, epoch: int, 
                       net_type: Literal['im2im', 'im2im_deep', "qutcc"]) -> Tuple[Path, str]:
    run_folder = Path(info["experiments_folder"]) / experiment_type / info[f"{net_type}_root"]
    analysis_dir = run_folder / "analysis"
    filename = f"quantile_qs_epoch_{epoch}.json" if net_type == "qutcc" else f"{net_type}_epoch{epoch}_lambda.pt"
    return analysis_dir, filename

def load_existing_checkpoint(experiment_type: str, info: Dict, epoch: int, 
                           net_type: Literal['im2im', 'im2im_deep', "qutcc"]) -> Optional[Dict[str, Any]]:
    try:
...
class CalibrateModelTask(submitit.helpers.Checkpointable):
    def __init__(self, experiment_type: str, info: Dict, epoch: int,
                 net_type: Literal['im2im', 'im2im_deep', "qutcc"], calib_subset: int = 2000):
    ...
    def __call__(self) -> Dict[str, Any]:
        ...
        calib_dataloader = get_calib_dataloader(
            ...
            net=self.net_type,
            batch_size_multiplier=1 if self.net_type != "qutcc" else 0.9,
            calib_subset=self.calib_subset
        )
        
        if self.net_type == "qutcc":
...
class CalibrationManager:
    ...
    def calibrate(self, net_type: Literal['im2im', 'im2im_deep', "qutcc"], 
...
def get_intervals_and_risk(net_type: Literal["im2im", "im2im_deep", "qutcc"],
